...
export default class EventManager {
   ...
   *   This is useful for platform customers who manage their own calendar events but still want Davnoot to create
  ...
  public async create(
    ...
    // Fallback to Davnoot Video if Google Meet is selected w/o a Google Calendar connection
    if (evt.location === MeetLocationType && mainHostDestinationCalendar?.integration !== "google_calendar") {
      ...
      if (!isDelegationCredential({ credentialId: googleCalendarCredential?.id })) {
        log.warn(
          "Falling back to Davnoot Video integration for Regular Credential as Google Calendar is not set as destination calendar"
        );
  ...
  private getVideoCredentialByCalendarEvent(event: CalendarEvent): CredentialForCalendarService | undefined {
    ...
    /**
     * This might happen if someone tries to use a location with a missing credential, so we fallback to Davnoot Video.
