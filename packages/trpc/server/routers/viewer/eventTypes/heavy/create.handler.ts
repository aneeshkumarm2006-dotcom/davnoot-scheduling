...
export const createHandler = async ({ ctx, input }: CreateOptions) => {
  ...
  try {
    ...
    // For team events, seed hosts with every accepted team member so the
    // round-robin/collective engine has participants immediately after creation.
    // Hosts can be refined afterwards in the event type's Team tab.
    if (teamId && schedulingType) {
