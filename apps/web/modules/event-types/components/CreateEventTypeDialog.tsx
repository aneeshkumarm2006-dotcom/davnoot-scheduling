...
export function CreateEventTypeDialog({ profileOptions }: { profileOptions: ProfileOption[] }) {
  ...
  // Team events require teamId + a schedulingType. Seed them into the form so the
  // create mutation targets the team (default Round Robin, editable in the selector).
  useEffect(() => {
    if (teamId) {
      form.setValue("teamId", teamId);
      if (!form.getValues("schedulingType")) {
        form.setValue("schedulingType", SchedulingType.ROUND_ROBIN);
      }
    }
